...
<Container>
  <ContentBox>
    <h2 class="text-6xl">Experience</h2>
  </ContentBox>
  <ul class="tag-cloud">
    {#each Object.entries(tags) as [tag, link]}
    <li>
      <a href={link}>{tag}</a>
    </li>
    {/each}
...
<style lang="postcss">
  ...
  .tag-cloud {
    @apply mx-auto max-w-[20rem];
    @media screen(md) {
      @apply max-w-[35rem] relative left-[50px];
    }
    li {
      @apply inline-block;
      @apply m-[0.4rem] mb-[0.8rem];
    }
    a {
